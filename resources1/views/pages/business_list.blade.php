...
@section('content')	
  ...
  @if($user == 'customer')
       ...
       @elseif($user == 'lead')
        	<div class="frame1-pm">
            <div class="btn-add-pm">
                <a href="/lead_all_request/pending/{{ $user_id }}"><button class="{{ strpos($url, 'pending') ? 'pending_complete_active' : 'pending_complete_deactive' }}">Pending Request</button></a>
            </div>
            <div class="btn-view-pm">
                <a href="/lead_all_request/completed/{{ $user_id }}"><button class="{{ strpos($url, 'completed') ? 'pending_complete_active' : 'pending_complete_deactive' }}">Completed Request</button></a>
            </div>
        </div>
